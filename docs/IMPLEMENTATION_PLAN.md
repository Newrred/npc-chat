...
2026-09-21 외부 검토 후속: Task27의 정확성 회귀 수정 뒤 Task28에서 원문을 저장하지 않는 유효 설정 manifest·단계별 지연 계측과 development/holdout 합성 검증 세트를 구현했다. development 24개 현재 기준선을 확보했으며 holdout은 보존한다. 다음 작은 작업은 동일 9B의 metadata 전용 축약 문맥을 이 기준선으로 A/B 비교하는 것이다. 테스터 확대 전에는 고정 주소·guest 지속성·두 DB 백업/복구·자동 복구/부하 게이트를 완료한다. 의미 검색은 현재 검색 실패 단계를 분리한 뒤 shadow 비교로만 시작한다.
