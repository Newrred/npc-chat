...
2026-09-21 외부 검토 후속: Task27에서 취소 의미 판정과 캐릭터별 근거 회상 말투의 정확성 회귀를 먼저 수정했다. 다음 작은 작업은 원문을 저장하지 않는 유효 설정 manifest·단계별 지연 계측·독립 검증 세트이며, 그 기준선으로 동일 9B의 metadata 전용 축약 문맥을 A/B 비교한다. 테스터 확대 전에는 고정 주소·guest 지속성·두 DB 백업/복구·자동 복구/부하 게이트를 완료한다. 의미 검색은 현재 검색 실패 단계를 분리한 뒤 shadow 비교로만 시작한다.
