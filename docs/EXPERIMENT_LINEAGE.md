...
## EXP-19 / Task26 — 띳띠 자유대화 프롬프트 완화 (계획)

가설: 정체성 첫 문장과 일반 규칙·예시에 반복 배치된 기사·방랑자·검·바람 표현이 9B의 일상 답변에도 원작 콘셉트를 과도하게 유도한다. 모델/4096/256/두 단계/샘플링/DB/API는 고정하고 `cartethyia.json`의 프롬프트만 변경한다. 본명·별명·다정하고 장난기 있는 성격·자연스러운 해요체는 유지한다. 원작 고유어는 사용자가 직접 묻거나 현재 화제와 명확히 관련 있을 때만 사용하고, 기본 방랑자 호칭을 제거하며 예시는 이름 1개와 일반 일상 대화 중심으로 바꾼다. 이름·일상 추천·감정 공감·취향·가벼운 잡담과 명시적 설정 질문을 실제 9B에서 확인한다. 판정은 본명/별명 보존, 해요체, 일상 사례의 불필요한 원작 고유어 0건, 설정 질문에는 정체성 보존이다. 합성 새 대화만 사용하고 DB·공개 quota는 사용하지 않는다.

EXP19 완료/적용: 초기 6개 단일 턴에서 이름 보존, 일상 4건의 원작 고유어 0건, 설정 질문의 리나시타 정체성을 확인했다. 가벼운 잡담 1건의 반말 종결과 추상적 제안을 발견해 해요체 종결 및 추천 시 구체 선택 규칙을 보강했다. 보강 뒤 추천·설정 3건과 4메시지 문맥 후속 1건은 모두 해요체였고 일상·문맥 3건에는 원작 고유어가 없었다. 추천의 구체성은 3건 중 2건에서 뚜렷했고 음식 1건은 범주 수준이라 소형 모델 변동성이 남는다. 현재 두 단계 reply는 EXAMPLES를 읽지 않으므로 운영 효과는 IDENTITY·CHARACTER 변경에서 나온다. EXAMPLES 수정은 single_pass rollback 일관성을 위한 것이다. 총 13개 로컬 합성 결정과 공개 구성 1회 smoke를 확인했다. 공개 smoke로 일일 quota 1회를 사용했고 생성한 합성 대화는 즉시 초기화했으며 기존 사용자 대화는 읽거나 쓰지 않았다.

## EXP-20 / EXP-19 — 독립 합성 개발 기준선 (계획)

가설: 외부 검토에서 제안한 8개 대화 축을 기존 소규모 반복 사례와 분리한 48개 합성 세트로 고정하면, 이후 metadata 문맥 축약의 품질과 지연을 같은 기준으로 비교할 수 있다. 현재 9B Q4_K_M/4096/256/two-stage/샘플링/프롬프트/검색·DB 구조를 고정한다. 각 범주의 1~3번인 development 24개만 현재 기준선에서 실행하고 4~6번 holdout 24개는 조정 중 실행하지 않는다. 형식·표정, 주체, 근거, 실질 답변, 자연스러움, 캐릭터 일관성의 사람 평가를 자동 문자열 보조지표와 분리한다. 실제 사용자 대화·DB·공개 API·일일 quota는 사용하지 않고 로컬 모델에 합성 입력만 순차 호출한다. 결과는 `.runtime` 원자료와 비개인 집계로 기록하며, 반복 실행을 독립 사례 수로 세지 않는다. Task28.

EXP20 현재 기준선 완료/변경 미적용: development 24개를 한 번씩 실행해 reply+metadata 48호출 모두 최초 형식 유효, parse·transport 실패0을 확인했다. 전체 중앙2.532초, 표본 p95 3.891초이며 reply 중앙0.860초, metadata 중앙1.617초다. 자동 문자열 보조검사는18/24였지만 경계3건의 무조건 수락을 모두 통과시키고 정상적인 모름/추천 일부를 실패로 잡아 품질 점수로 사용할 수 없음을 확인했다. 단일 검토에서 추천 이유 누락, 사용자 이름 답변의 유이 말투 이탈, 못 먹는 취향의 의미 약화, 시간 회상의 중국어 혼입, 경계3건 실패, 일부 부자연스러운 말투를 발견했다. 이름 주체 구분, 저장 취향·책·긴 대화 뒤 장소 회상, 기억 부재 처리는 대체로 작동했다. holdout은 실행하지 않았고 운영 프롬프트·모델·DB·API는 변경하지 않았다. 첫 실행은 도구의 import 경로 누락으로 모델 호출 전 중단되어 CLI 회귀를 추가한 뒤 재실행했다. [결과와 한계](DIALOGUE_SUITE_BASELINE.md), [합성 원자료](evaluation/dialogue-suite-development-baseline-v1.json).

## EXP-21 / EXP-20 — metadata 전용 문맥 경량화 A/B (계획)

가설: reply 입력은 그대로 두고 metadata 입력을 현재 사용자 발언, 확정 대사, 관계/flags, 직전 완전 대화 최대2쌍과 선택된 profile/episode 근거로 한정하면, metadata의 분류·기억 후보·주체·표정 품질을 유지하면서 입력 token과 지연을 줄일 수 있다. A는 현재 full context, B는 compact context다. 9B Q4_K_M/4096/256/two-stage/샘플링/프롬프트/schema/검색/DB를 고정하고 development 24개만 사용하며 holdout은 실행하지 않는다. 형식·interaction·memory candidate·flags·face/emotion과 reply 품질, prompt token, prepare/infer/전체 지연을 분리한다. 실제 사용자 DB·공개 API·quota는 사용하지 않는다. B가 불리하면 full 기본값을 유지하고, 비열화 없이 명확한 입력 감소가 확인될 때만 공개 시험 설정을 compact로 바꾼다. Task29.

EXP21 완료/compact 미채택: 최초 전체 two-stage 실행은 reply 변동으로 인과 비교에 부적합해 진단 자료로만 두고, EXP20의 동일 reply를 고정한 metadata 24쌍을 조건 순서 교대로 재실행했다. 양쪽24/24 최초 형식 성공, parse·transport 실패0. prompt 중앙903.5→900, 평균956.9→925.4, metadata 전체 중앙2.062→2.094초로 일반적인 속도 이득이 없었다. 24쌍 긴 이력1건은1739→1080 token, infer2.656→2.296초로 개선됐다. interaction 차이7건 중 compact는 짧은 수락·기억 질문·자기 공개에서 더 자주 부적절했고, 무조건 복종 요구는 양쪽 모두 실패했다. 원시 기억 후보는12/11사례였지만 서버 최종 수용은 양쪽0건. 공개/default는 full 유지, compact는 명시적 실험 옵션으로만 보존한다. holdout·사용자DB·공개quota 미사용. [결과](METADATA_CONTEXT_AB.md), [원자료](evaluation/metadata-context-ab-v1.json).

## EXP-22 / EXP-21 — TokenCounter 연결 재사용·요청 내부 캐시 (완료·적용)

가설: llama.cpp apply-template/tokenize의 정확한 검증을 그대로 두고 HTTP client/연결을 TokenCounter 생명주기 동안 재사용하며, 한 최상위 생성 요청 안의 완전히 같은 messages만 해시 기반으로 재사용하면 reply/metadata 내용과 호출 구조를 바꾸지 않고 prepare 지연을 줄일 수 있다. 변경 전후 같은 합성 메시지의 독립 호출과 request scope 중복 호출을 측정한다. 정상 two-stage의 서로 다른 system prompt를 같은 값으로 취급하지 않는다. 9B/full/4096/256/샘플링/DB/API를 고정하고 사용자DB·공개 quota 없이 단위·전체 검증 후, 채택 시 full 공개 smoke를 사용한다. 정확성, HTTP 요청 수, client 생성 수, prepare/전체 지연을 분리한다. Task30.

결과: 동일 입력 합성20요청에서 client40→1, HTTP80→40, 12.003→0.582초였고 token 결과35는 같았다. 서로 다른 reply/metadata 모사에서는 client40→1, HTTP80 유지, 11.744→0.388초이며 결과25/28이 같아 정상 경로의 주효과가 연결 재사용임을 확인했다. 실제 development24개는 전후 모두 최초 형식24/24·parse/transport0이며 prepare p50 0.578→0.297초였다. 모델 생성 변동으로 decision 전체 p50은 2.531→3.546초라 전체 응답 속도 향상으로 판정하지 않는다. 공개 연속2턴은 tokenizer4/cache hit0을 유지하면서 첫 턴 reply/metadata prepare 0.297/0.000초, 다음 턴 0.000/0.015초였고 reset했다. 입력 준비 최적화로 적용. [상세](TOKEN_COUNTER_REUSE.md).
