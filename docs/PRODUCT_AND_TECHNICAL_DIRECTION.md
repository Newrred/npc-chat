...
현재 기본 구현은 FastAPI 단일 인스턴스·단일 worker의 메모리 잠금과 제한된 생성 대기열을 사용하고, 내구성이 필요한 세션 연결·대화·관계·기억·중복 처리는 SQLite에 저장한다. Phase 03의 영속 저장·동시성·중복 처리·마이그레이션 검증을 통과했으며 기본 실행에 Redis는 필요 없다. Redis는 이후 다중 프로세스 운영 등이 필요할 때 명시적으로 선택하는 확장 수단이다.
...
- Phase 01의 화면/API 통합과 `start-local.ps1` / `stop-local.ps1`, Phase 03의 SQLite 전환을 완료했다.
- SQLite 저장, 프로필/캐릭터별 동시 처리 직렬화, 전역 제한 대기열, DB 기반 중복 방지를 검증했으며 Redis를 기본 실행에서 제외했다.
