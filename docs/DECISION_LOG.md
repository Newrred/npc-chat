...
## 2026-09-22 모바일에서는 비동기 답변 후 입력창을 자동 포커스하지 않음

모바일 가상 키보드가 전송 때 내려갔다가 답변 도착과 함께 다시 열리는 반복을 막는다. 답변 완료 시 자동 포커스는 hover 가능한 fine pointer 환경에만 적용한다. 모바일 사용자는 다음 메시지를 쓸 때 입력창을 직접 누르며, `내용 수정`처럼 사용자가 명시적으로 입력을 요청한 동작은 계속 포커스한다. Task31 참고.
...
## 2026-09-21 취소 의미와 근거 회상 말투를 결정 규칙으로 일치

약속 취소 저장과 최근 활성 화제 절단은 같은 취소 판정을 사용한다. 취소 부정·질문·인용은 화제를 지우지 않고, 실제 취소 뒤 같은 발화의 새 제안은 새 사건으로 보존한다. 취향 회상의 근거 선택과 80자 제한은 공통 서버 로직에 두되 최종 짧은 표현은 캐릭터 설정의 `grounded_recall_template`을 사용한다. 추가 모델 호출이나 DB migration은 도입하지 않는다. Task27 참고.

## 2026-09-11 띳띠는 일상 대화를 우선하고 세계관 설정은 필요할 때만 사용

본명·별명과 핵심 성격은 유지하되 평범한 대화에서는 현재 주제와 사용자의 말을 우선한다. 리나시타·기사·검·플뢰르드리스·방랑자 같은 설정 표현은 사용자가 직접 묻거나 현재 화제와 분명히 관련 있을 때만 사용한다. 사용자에게 기본적으로 `방랑자`라고 부르지 않으며, 알려 준 이름이나 호칭만 사용한다. 생성 구조·모델·DB·API는 변경하지 않는다.

## 2026-09-11 두 번째 캐릭터의 사용자 표시 이름은 띳띠

두 번째 캐릭터의 본명은 `카르티시아`, 더 자주 불리는 별명은 `띳띠`로 설정한다. 대화 목록과 채팅 화면은 별명 `띳띠`를 표시하고, 모델은 이름 질문에 본명과 별명을 함께 설명한다. 이미 생성된 캐릭터별 저장 데이터와 정적 이미지 URL의 호환성을 위해 내부 `character_id`와 경로 `cartethyia`는 유지한다. 따라서 이름 설정 변경만으로 DB migration이나 기존 대화 초기화는 발생하지 않는다.

## 2026-09-11 카르티시아를 두 번째 대화 캐릭터로 추가

사용자 제공 17개 표정 세트를 사용해 카르티시아를 실제 선택 가능한 두 번째 방으로 추가한다. 새 모델이나 DB schema를 만들지 않고 기존 캐릭터 설정 로더와 `(profile_id, character_id)` 저장 경계를 API/UI까지 연결한다. `character_id`는 허용된 형식과 존재하는 설정 파일로 제한하며 생략 시 `default` 유이를 유지한다. 프로필은 사용자 단위로 공유하고 세션·대화·관계·기억·pending 요청은 캐릭터별로 분리한다. 카르티시아 프롬프트는 공식 캐릭터 소개의 상냥함·용기·유랑 기사 설정과 일상 대화용 규칙만 사용하며 전체 원작 지식베이스를 주장하지 않는다. Task26 참고.
